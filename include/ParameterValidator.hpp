...
public:
  ...
  struct ValidationResult {
    ...
    ValidationResult(bool valid, std::string path, std::string val, 
        ...
        : isValid(valid), paramPath(std::move(path)), value(std::move(val)),
          errorMessage(std::move(error)), warningMessage(std::move(warning)) {}
  };
  ...
  struct ValidationSummary {
    ...
    double GetValidationRate() const {
      return totalParameters > 0 ? 
        static_cast<double>(validParameters) / totalParameters : 0.0;
    }
  };
  ...
  using CustomValidator = std::function<ValidationResult(const std::string& path, 
  ...
  ValidationSummary ValidateParameters(const std::vector<std::array<std::string, 2>>& config);
  ValidationResult ValidateParameter(const std::string& paramPath, const std::string& value);
  ValidationResult ValidateSingleParameter(const std::string& paramPath, const std::string& value);

  // Validation configuration
  void SetAllowUnknownParameters(bool allow) { allowUnknownParameters_ = allow; }
  ...
  void AddCustomValidator(const std::string& paramPattern, CustomValidator validator);
  ...
  std::optional<std::string> GetParameterDescription(const std::string& paramPath) const;
  std::optional<std::pair<std::string, std::string>> GetParameterRange(const std::string& paramPath) const;
  ...
  std::vector<std::string> GetChannelParameters(const std::string& channel) const;
  ...
  void ExportValidationResults(const ValidationSummary& summary, const std::string& filename) const;

private:
  // Data members
  const nlohmann::json& deviceTree_;
  bool allowUnknownParameters_ = true;
  ...
  std::vector<std::string> ExpandChannelRange(const std::string& configPath) const;
  ...
  nlohmann::json GetChannelParameterDefinition(const std::string& channel, 
