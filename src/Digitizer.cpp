...
bool Digitizer::ResetDigitizer()
{
  return SendCommand("/cmd/Reset");
}

bool Digitizer::ApplyConfiguration()
{
  bool status = true;
...
bool Digitizer::ConfigureRecordLength()
{
  std::string buf;
...
bool Digitizer::ConfigureMaxRawDataSize()
{
  std::string buf;
...
bool Digitizer::InitializeDataConverter()
{
...
bool Digitizer::ConfigureSampleRate()
{
  std::string buf;
...
bool Digitizer::StartAcquisition()
{
  ...
  for (uint32_t i = 0; i < fNThreads; i++) {
    fReadDataThreads.emplace_back(&Digitizer::ReadDataThread, this);
  }

  // Start single EventData conversion thread
  fEventConversionThread = std::thread(&Digitizer::EventConversionThread, this);
...
bool Digitizer::EndpointConfigure()
{
  // Configure endpoint
  uint64_t epHandle;
...
int Digitizer::ReadDataWithLock(std::unique_ptr<RawData_t> &rawData,
                                int timeOut)
{
...
nlohmann::json Digitizer::GetReadDataFormatRAW()
{
...
bool Digitizer::CheckError(int err)
{
...
bool Digitizer::SendCommand(const std::string &path)
{
...
bool Digitizer::GetParameter(const std::string &path, std::string &value)
{
  char buf[256];
...
bool Digitizer::SetParameter(const std::string &path, const std::string &value)
{
...
bool Digitizer::Open(const std::string &url)
{
...
std::string Digitizer::GetDeviceTree()
{
  if (fHandle == 0) {
...
void Digitizer::DetermineDigitizerType()
{
...
bool Digitizer::ValidateParameters()
{
...
void Digitizer::EventConversionThread()
{
  while (fDataTakingFlag) {
    // Poll RawToPSD2 for converted data
    auto psd2DataVec = fRawToPSD2 ? fRawToPSD2->GetData() : nullptr;

    if (psd2DataVec && !psd2DataVec->empty()) {
      // Convert all PSD2 data to EventData in batch
      std::vector<std::unique_ptr<EventData>> eventDataBatch;
      eventDataBatch.reserve(psd2DataVec->size());

      for (auto &psd2Data : *psd2DataVec) {
        if (psd2Data) {
          auto eventData = ConvertPSD2ToEventData(*psd2Data);
          if (eventData) {
            eventDataBatch.push_back(std::move(eventData));
          }
        }
      }

      // Add batch to EventData storage
      if (!eventDataBatch.empty()) {
        std::lock_guard<std::mutex> lock(fEventDataMutex);
        if (fEventDataVec) {
          fEventDataVec->insert(fEventDataVec->end(),
                                std::make_move_iterator(eventDataBatch.begin()),
                                std::make_move_iterator(eventDataBatch.end()));
        }
      }
    } else {
...
std::unique_ptr<EventData> Digitizer::ConvertPSD2ToEventData(
    const PSD2Data_t &psd2Data)
{
  auto eventData = std::make_unique<EventData>(psd2Data.waveformSize);

  // Copy basic timing and energy information
  eventData->timeStampNs = psd2Data.timeStampNs;
  eventData->SetTimeStampNs(psd2Data.timeStampNs);
  eventData->energy = psd2Data.energy;
  eventData->SetEnergy(psd2Data.energy);
  eventData->energyShort = psd2Data.energyShort;
  eventData->SetEnergyShort(psd2Data.energyShort);

  // Copy channel and module information
  eventData->channel = psd2Data.channel;
  eventData->SetChannel(psd2Data.channel);
  eventData->module = fModuleNumber;  // Set from digitizer module number
  eventData->SetModule(fModuleNumber);

  // Copy time resolution and sampling information
  eventData->timeResolution = psd2Data.timeResolution;
  eventData->SetTimeResolution(psd2Data.timeResolution);
  eventData->downSampleFactor = psd2Data.downSampleFactor;
  eventData->SetDownSampleFactor(psd2Data.downSampleFactor);

  // Copy probe type information
  eventData->analogProbe1Type = psd2Data.analogProbe1Type;
  eventData->SetAnalogProbe1Type(psd2Data.analogProbe1Type);
  eventData->analogProbe2Type = psd2Data.analogProbe2Type;
  eventData->SetAnalogProbe2Type(psd2Data.analogProbe2Type);
  eventData->digitalProbe1Type = psd2Data.digitalProbe1Type;
  eventData->SetDigitalProbe1Type(psd2Data.digitalProbe1Type);
  eventData->digitalProbe2Type = psd2Data.digitalProbe2Type;
  eventData->SetDigitalProbe2Type(psd2Data.digitalProbe2Type);
  eventData->digitalProbe3Type = psd2Data.digitalProbe3Type;
  eventData->SetDigitalProbe3Type(psd2Data.digitalProbe3Type);
  eventData->digitalProbe4Type = psd2Data.digitalProbe4Type;
  eventData->SetDigitalProbe4Type(psd2Data.digitalProbe4Type);

  // Move waveform data efficiently (avoid copies)
  eventData->waveformSize = psd2Data.waveformSize;
  if (psd2Data.waveformSize > 0) {
    // Copy once to both legacy public and private members
    eventData->analogProbe1 = psd2Data.analogProbe1;
    eventData->analogProbe2 = psd2Data.analogProbe2;
    eventData->digitalProbe1 = psd2Data.digitalProbe1;
    eventData->digitalProbe2 = psd2Data.digitalProbe2;
    eventData->digitalProbe3 = psd2Data.digitalProbe3;
    eventData->digitalProbe4 = psd2Data.digitalProbe4;

    // Set private members using move to avoid second copy
    eventData->SetAnalogProbe1(std::vector<int32_t>(eventData->analogProbe1));
    eventData->SetAnalogProbe2(std::vector<int32_t>(eventData->analogProbe2));
    eventData->SetDigitalProbe1(std::vector<uint8_t>(eventData->digitalProbe1));
    eventData->SetDigitalProbe2(std::vector<uint8_t>(eventData->digitalProbe2));
    eventData->SetDigitalProbe3(std::vector<uint8_t>(eventData->digitalProbe3));
    eventData->SetDigitalProbe4(std::vector<uint8_t>(eventData->digitalProbe4));
  }
