...
bool RawToPSD2::ValidateDataHeader(uint64_t headerWord, size_t dataSize)
{
  ...
  if (headerType != kHeaderTypeData) {
    std::cerr << "Invalid header type: 0x" << std::hex << headerType << std::dec << std::endl;
    return false;
  }
  ...
  auto aggregateCounter = (headerWord >> kAggregateCounterShift) & kAggregateCounterMask;
...
void RawToPSD2::ProcessEventData(const std::vector<uint8_t>::iterator& dataStart, 
                                uint32_t totalSize)
{
                               ...
                               size_t& wordIndex, PSD2Data_t& psd2Data)
{
  ...
  std::memcpy(&firstWord, &(*(dataStart + wordIndex * kWordSize)), sizeof(uint64_t));
  wordIndex++;
  
  // Read second word (flags and energy)
  uint64_t secondWord = 0;
  std::memcpy(&secondWord, &(*(dataStart + wordIndex * kWordSize)), sizeof(uint64_t));
  wordIndex++;
...
{
  // Extract flags
  psd2Data.flagsLowPriority = (word >> kFlagsLowPriorityShift) & kFlagsLowPriorityMask;
  psd2Data.flagsHighPriority = (word >> kFlagsHighPriorityShift) & kFlagsHighPriorityMask;
  ...
  if (fDumpFlag) {
    std::cout << "Low priority flags: " << psd2Data.flagsLowPriority << std::endl;
    std::cout << "High priority flags: " << psd2Data.flagsHighPriority << std::endl;
    std::cout << "Short gate: " << psd2Data.energyShort << std::endl;
    std::cout << "Energy: " << psd2Data.energy << std::endl;
    std::cout << std::fixed << std::setprecision(3);
    std::cout << "Final timestamp: " << psd2Data.timeStampNs << " ns" << std::endl;
    std::cout << std::defaultfloat;
  }

}

void RawToPSD2::DecodeWaveformData(const std::vector<uint8_t>::iterator& dataStart,
                                  size_t& wordIndex, PSD2Data_t& psd2Data)
{
  // Read waveform header
  uint64_t waveformHeader = 0;
  std::memcpy(&waveformHeader, &(*(dataStart + wordIndex * kWordSize)), sizeof(uint64_t));
  wordIndex++;
  
  // Validate waveform header
  bool headerValid = ((waveformHeader >> kWaveformCheck1Shift) & 0x1) == 0x1 &&
  ...
  std::memcpy(&nWordsWaveform, &(*(dataStart + wordIndex * kWordSize)), sizeof(uint64_t));
  wordIndex++;
  ...
  for (size_t j = 0; j < nWordsWaveform; j++) {
    ...
    std::memcpy(&waveformWord, &(*(dataStart + wordIndex * kWordSize)), sizeof(uint64_t));
    wordIndex++;
...
void RawToPSD2::DecodeWaveformHeader(uint64_t header, PSD2Data_t& psd2Data) const
{
  ...
  psd2Data.triggerThr = (header >> kTriggerThresholdShift) & kTriggerThresholdMask;
...
RawToPSD2::WaveformConfig RawToPSD2::ExtractWaveformConfig(uint64_t header) const
{
                                   ...
                                   const WaveformConfig& config, PSD2Data_t& psd2Data) const
{
  ...
  if (config.ap1IsSigned) {
    ...
    psd2Data.analogProbe1[dataIndex] = static_cast<int32_t>(analog1Raw) * config.ap1MulFactor;
  } else {
  ...
  if (config.ap2IsSigned) {
    ...
    psd2Data.analogProbe2[dataIndex] = static_cast<int32_t>(analog2Raw) * config.ap2MulFactor;
  } else {
