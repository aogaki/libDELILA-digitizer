    ...
    if (!configPair[0].empty() && configPair[0][0] == '/') {
      ...
      if (paramPath.find("..") != std::string::npos) {
        ...
        if (!expandedPaths.empty()) {
          ...
          result = ValidationResult(false, paramPath, paramValue, "Failed to expand channel range");
        }
      } else {
...
ParameterValidator::ValidationResult ParameterValidator::ValidateSingleParameter(
    const std::string& paramPath, const std::string& value) {
  ...
  if (paramDef.empty()) {
    if (allowUnknownParameters_) {
      return ValidationResult(true, paramPath, value, "", "Parameter not found in device tree");
    } else {
      return ValidationResult(false, paramPath, value, "Parameter not found in device tree");
    }
  }
...
void ParameterValidator::AddCustomValidator(const std::string& paramPattern, CustomValidator validator) {
...
void ParameterValidator::ClearCustomValidators() {
...
ParameterValidator::ParameterType ParameterValidator::GetParameterType(const std::string& paramPath) const {
  auto paramDef = GetParameterDefinition(paramPath);
  return ParseParameterType(paramDef);
}

std::optional<std::string> ParameterValidator::GetParameterDescription(const std::string& paramPath) const {
  auto paramDef = GetParameterDefinition(paramPath);
  if (paramDef.contains("description") && paramDef["description"].contains("value")) {
    return paramDef["description"]["value"].get<std::string>();
  }
  return std::nullopt;
}

bool ParameterValidator::IsParameterSupported(const std::string& paramPath) const {
...
std::vector<std::string> ParameterValidator::ExpandChannelRange(const std::string& configPath) const {
...
std::string ParameterValidator::MapConfigToDeviceTree(const std::string& configParam) const {
...
bool ParameterValidator::MatchesPattern(const std::string& path, const std::string& pattern) const {
  try {
...
ParameterValidator::ValidationResult ParameterValidator::ValidateParameterValue(
    const nlohmann::json& paramDef, const std::string& paramPath, const std::string& value) const {
  ...
  switch (type) {
    ...
    default:
      return ValidationResult(true, paramPath, value, "", "Unknown parameter type");
  }
}

ParameterValidator::ValidationResult ParameterValidator::ValidateNumberParameter(
    const nlohmann::json& paramDef, const std::string& paramPath, const std::string& value) const {
  try {
    double numValue = std::stod(value);
    
    if (paramDef.contains("minvalue") && paramDef["minvalue"].contains("value")) {
      double minVal = std::stod(paramDef["minvalue"]["value"].get<std::string>());
      if (numValue < minVal) {
        return ValidationResult(false, paramPath, value, 
          "Value " + value + " below minimum: " + paramDef["minvalue"]["value"].get<std::string>());
      }
    }

    if (paramDef.contains("maxvalue") && paramDef["maxvalue"].contains("value")) {
      double maxVal = std::stod(paramDef["maxvalue"]["value"].get<std::string>());
      if (numValue > maxVal) {
        return ValidationResult(false, paramPath, value,
          "Value " + value + " above maximum: " + paramDef["maxvalue"]["value"].get<std::string>());
      }
    }
...
ParameterValidator::ValidationResult ParameterValidator::ValidateIntegerParameter(
    const nlohmann::json& paramDef, const std::string& paramPath, const std::string& value) const {
  try {
...
ParameterValidator::ValidationResult ParameterValidator::ValidateStringParameter(
    const nlohmann::json& paramDef, const std::string& paramPath, const std::string& value) const {
  // Basic string validation - can be extended for allowed values
  return ValidationResult(true, paramPath, value);
}

ParameterValidator::ValidationResult ParameterValidator::ValidateBooleanParameter(
    const nlohmann::json& paramDef, const std::string& paramPath, const std::string& value) const {
...
nlohmann::json ParameterValidator::GetParameterDefinition(const std::string& paramPath) const {
...
nlohmann::json ParameterValidator::GetChannelParameterDefinition(
  ...
  if (deviceTree_.contains("ch") && 
      deviceTree_["ch"].contains(channel) &&
...
nlohmann::json ParameterValidator::GetRootParameterDefinition(const std::string& paramName) const {
...
std::string ParameterValidator::FormatValidationMessage(const ValidationResult& result) const {
...
ParameterValidator::ParameterType ParameterValidator::ParseParameterType(const nlohmann::json& paramDef) const {
  if (paramDef.contains("datatype") && paramDef["datatype"].contains("value")) {
    ...
    if (dataType == "boolean" || dataType == "bool") return ParameterType::Boolean;
...
void ParameterValidator::PrintValidationReport(const ValidationSummary& summary) const {
  std::cout << "=============================" << std::endl;
...
std::string ParameterValidator::GenerateValidationReport(const ValidationSummary& summary) const {
...
void ParameterValidator::ExportValidationResults(const ValidationSummary& summary, const std::string& filename) const {
